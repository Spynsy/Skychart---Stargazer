...
def draw(obs_loc, t, mag_max=5, alpha=0.3, figsize=None, zoom_az=None, zoom_alt=None):
    df = visible_hipparcos(obs_loc, t)
    df_show = df[df['Vmag'] < mag_max]
    dc_const = load_constellations()
    fig, ax, _ = draw_chart(df, df_show, dc_const, alpha, figsize)
    plot_altaz(df_show['az'], df_show['alt'], mag=df_show['Vmag'], figsize=figsize,
               zoom_az=zoom_az, zoom_alt=zoom_alt, ax=ax)
    return fig, ax


def plot_altaz(az, alt, mag=None, size=None, color='w', alpha=1, marker='o', ax=None, figsize=None, 
               zoom_az=None, zoom_alt=None):
    """
    Plot positions of bodies based on altitude/azimuth coordinates with optional zoom functionality.
    Arguments
    ---------
        ...
        zoom_az (tuple of float): azimuth range for zoom (min_az, max_az); default None.
        zoom_alt (tuple of float): altitude range for zoom (min_alt, max_alt); default None.
    Returns
    -------
    ...
    if zoom_az:
        ax.set_xlim(np.radians(zoom_az[0]), np.radians(zoom_az[1]))
    if zoom_alt:
        ax.set_ylim(90 - zoom_alt[1], 90 - zoom_alt[0])
